"""
CamFollower class and equations for H-PDEKF
"""
...
#----------------------------------------------------------------------------------------------------------------------#
class CamFollower():
    def __init__(self, HB_trainable=False, m_trainable=False, V_trainable=False, include_friction_term=False, nq=None):
        ...
        # assert(sum([HB_trainable, m_trainable, V_trainable]) <= 1), "Maximum one of the parameters can be trainable"
...
def f_func(th,omega,V,m,H,beta,z,p, include_friction_term=False):
    ...
    if include_friction_term:
        omega_dot =  (T_shaft+NN-fric_rest-Tl_eq)/J_eq
    else:
        omega_dot =  (T_shaft+NN-Tl_eq)/J_eq
