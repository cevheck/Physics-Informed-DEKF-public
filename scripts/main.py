"""
Main file to choose which ensemble to simulate/save/plot
"""
#%% imports 
...
#%% variables to set
overwrite = False       # overwrite existing results True/False
plot_now = False         # plot results now True/False
save_now = True        # save results True/False
            ...
            for V_i in V_list:              
                ...
                for model in modelopts:
                    for Kalman_method in kalman_opts:
                        ...
                        t1 = time.time()                    
                        processes = []
                        if False:    # perform with multiprocessing (ensures each dekf instance correctly releases memory)
                            queue = Queue()
